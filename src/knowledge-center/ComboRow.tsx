...
export const GRID_COLS = "1.6fr 0.6fr 0.6fr 0.6fr 0.7fr 1.8fr 20px";
...
export function ComboTableHeader() {
  return (
    <div style={{ display: "grid", gridTemplateColumns: GRID_COLS, gap: 10, padding: "10px 16px", background: "var(--color-surface)", borderBottom: "1px solid var(--color-border)" }}>
      ...
      <span style={HEADER_CELL}>ICP</span>
      <span style={HEADER_CELL}>Persona</span>
      <span style={HEADER_CELL}>Product</span>
      <span style={HEADER_CELL}>Overall</span>
...
export function ComboRow({ combo, expanded, onToggle, subtitleOverride }: { combo: CampaignCombo; expanded: boolean; onToggle: () => void; subtitleOverride?: string }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid var(--color-border)" }}>
      <button type="button" onClick={onToggle} className="kc-list-row"
        ...
        <span style={{ fontFamily: MONO, fontSize: 13, fontWeight: 700, color: tone(combo.icpScore) }}>{combo.icpScore.toFixed(1)}</span>
        <span style={{ fontFamily: MONO, fontSize: 13, fontWeight: 700, color: tone(combo.personaScore) }}>{combo.personaScore.toFixed(1)}</span>
        <span style={{ fontFamily: MONO, fontSize: 13, fontWeight: 700, color: tone(combo.productScore) }}>{combo.productScore.toFixed(1)}</span>
        ...
        <span style={{ color: "var(--color-muted)", transform: expanded ? "rotate(180deg)" : "none", transition: "transform 150ms var(--ease-apple)", display: "flex" }}>
          <Icon name="chevron-down" size={14} />
        </span>
      </button>
      {expanded && (
        <div style={{ padding: "0 16px 16px 16px" }}>
          <p style={{ margin: 0, fontSize: 12.5, color: "var(--color-body)", lineHeight: 1.6, background: "var(--color-surface)", borderRadius: 10, padding: "12px 14px" }}>{combo.detail}</p>
        </div>
      )}
    </div>
  );
